...
async def handler(ws):
    ...
    try:
        ...
        if role == "PEER":
            # the first client may have already sent its handshake message
            # before we joined -- that message had nowhere to go and was
            # dropped. Tell it a peer has now arrived so it can resend.
            for peer in peers:
                if peer is not ws:
                    await peer.send("PEERJOINED:")
